export function Memoize(propsToWatch?: string[]) {
  const hasPropsToWatch =
    typeof propsToWatch !== 'undefined' && Array.isArray(propsToWatch);
  ...
  return function (
    ...
    descriptor.value = function (this: any, ...args: any[]) {
      ...
      if (hasPropsToWatch) {
        const filteredEntries = Object.entries(this).filter(([k, v]) =>
          propsToWatch.includes(k)
        );
        modelKeyPart = Object.fromEntries(filteredEntries);
      }
      const reflectKey = `${JSON.stringify(modelKeyPart)}_${JSON.stringify(
        args
      )}_${propertyKey}`;
