...
export abstract class Serde<S> extends Object {
  protected removeableProperty(key: string): boolean {
    return (
      ...
      !(Reflect.getMetadata(EXCLUDED_PROPERTIES_KEY, this) || []).includes(key)
    );
  }
  ...
  serialize() {
    const pluckProperties = Reflect.getMetadata(
      PLUCK_PROPERTIES_KEY,
      this
    ) as PluckProperties;
    const serializedObj = Object.entries(this)
          ...
          if (typeof properties !== 'undefined') {
            if (Array.isArray(value)) {
              obj[key] = Array.isArray(properties)
                ? value.map((v) =>
                    properties.reduce((nv, p) => ({ ...nv, [p]: v[p] }), {})
                  )
    ...
    return typeof this.customSerialize === 'function'
      ? this.customSerialize(serializedObj)
      : serializedObj;
  }
}

export class EntityMetadata<S> extends Serde<S> {
  id: number;
  @Exclude() created: Date;
  @Exclude() createdBy?: number;
  @Exclude() updated: Date;
  @Exclude() updatedBy?: number;
}
